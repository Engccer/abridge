...
- PDF·스캔·이미지·양식: `docparse` 스킬로 파싱( https://github.com/Engccer/docparse )
...
**이 비율은 요약본과 원문이 같은 언어일 때만 문자 수로 잰다.** 언어마다 같은 내용을 담는 문자 수가 다르므로, 번역 후처리를 얹으면 번역본의 원문 대비 문자 비율은 목표 구간을 벗어난다. 실측 예: 영어 원문을 25%로 축약한 뒤 한국어로 옮기면 영어 원문 대비 13% 안팎이 된다(한국어가 같은 내용을 영어의 절반 정도 문자로 표현한다). 이때 압축이 더 된 것이 아니라 언어가 바뀐 것뿐이므로, 번역본을 보고 "덜 줄였다"거나 "너무 줄였다"고 판단해 다시 손대지 않는다. 분량 목표의 판정 대상은 언제나 **원문 언어 축약본**이다. 사용자가 비율을 지정하면 어느 언어 기준인지 확인하고, 확인이 어려우면 원문 언어 기준으로 잡고 그 사실을 밝힌다.
...
- **화자가 여럿인 전사본은 `화자: 발언` 형식으로 쓴다. 단 원문에 화자 라벨이 있을 때만이다.** 라벨이 없으면 산문으로 쓴다(이유는 `references/techniques.md` §3).
- **산출물이 마크다운으로 렌더링되지 않는 형식(`.txt`, 평문, TTS 입력 등)이면 강조 기호(`**`, `*`)를 쓰지 않는다.** 렌더링되지 않으면 별표가 글자 그대로 남아 화면에 노출되고 스크린 리더가 그대로 낭독한다. 화자 이름 강조는 `이름:` 콜론만으로 충분하다. 저장 형식을 모르면 강조를 넣지 않는 쪽이 안전하다.
...
- **1:1인지 기계로 확인한다.** 번역 후 원문 언어 축약본과 번역본의 섹션 수, 문단 수, 직접 인용 개수, 화자 대사 줄 수를 세어 일치하는지 대조한다. 하나라도 줄었으면 번역 중에 재요약이 끼어든 것이므로 그 자리를 찾아 복원한다. 문자 수 비율은 언어가 다르면 어차피 달라지므로 이 대조의 기준이 되지 못한다(위 "분량 기본값" 참조).
...
이 스킬에 동봉된 `scripts/elevenlabs_tts.py`를 호출한다. TTS를 새로 구현하거나 다른 스킬을 추가로 설치할 필요가 없다(원본은 speech-toolkit 저장소의 동일 스크립트이며 사본을 동봉했다: https://github.com/Engccer/speech-toolkit ).
